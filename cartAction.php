...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){ 
    if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){ 
        ...
        // Get product details 
        $query = $conn->query("SELECT * FROM tbl_shoppe WHERE id=$productID"); 
        ...
        $redirectLoc = $insertItem?'viewCart.php':'product_page.php'; 
        ...
        if(empty($errorMsg)){ 
            ...
            if($insertCust){ 
                ...
                $insertOrder = $conn->query("INSERT INTO orders (customer_id, grand_total, created, status, pay_type) VALUES ($custID, '".$cart->total()."'+100, NOW(), 'Pending', 'COD')"); 
